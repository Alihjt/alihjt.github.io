---
title:  "HydraViT: Stacking Heads for a Scalable ViT"
date:   2024-09-26 00:00:00 +00:00
image: /publications/images/hydravit.gif
author: "J Haberer, A Hojjat, O Landsiedel"
authors: "<strong>J Haberer</strong>, <strong>A Hojjat</strong>, O Landsiedel"
venue: "Presented at NeurIPS 2024 (poster)"
bib: |
  @inproceedings{Haberer2024HydraViT,
    author = {Haberer, Janek and Hojjat, Ali and Landsiedel, Olaf},
    title = {HydraViT: Stacking Heads for a Scalable ViT},
    booktitle = {Advances in Neural Information Processing Systems},
    volume = {37},
    pages = {40254--40277},
    year = {2024},
    month = {December}
  }
links:
  Code: https://github.com/ds-kiel/HydraViT
  arXiv: https://arxiv.org/abs/2409.17978
---
HydraViT introduces a **stacked‑head stochastic training** method for Vision Transformers, producing a single model that embeds up to **10 subnetworks**, each with varying head counts. It adapts dynamically to hardware constraints, yielding an ImageNet‑1K accuracy boost of up to **5 pp at equal GMACs** and **7 pp at equal throughput** versus state‑of‑the‑art baselines.  [oai_citation:0‡arxiv.org](https://arxiv.org/abs/2409.17978?utm_source=chatgpt.com)